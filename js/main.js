...
--------------------------------------------------------------*/
		...
		if ($("#marshall-form").is_exist()) {
			...
			mform.ajaxChimp({callback: callbackFunction, url: 'http://xyz.us14.list-manage.com/subscribe/post?u=3e624ea3457b50d638f1bd58b&id=bc67dbaebe'});
...
--------------------------------------------------------------*/
		if ($("#mrs_particles_can").is_exist()) {
			$('#mrs_particles_can').particleground({dotColor: '#FFF', lineColor: '#FFF'});
		}
...
--------------------------------------------------------------*/
		...
		if (ripple.is_exist()) {
			ripple.ripples({
				resolution: 512, dropRadius: 10, //px
				perturbance: 0.04,
				interactive: false
			});
